            ...
            while *running.read().await {
                ...
                if let Some(task) = consensus_client.poll_task().await {
                    ...
                    tokio::spawn(async move {
                        ...
                        // 如果是作弊行为，记录安全事件
                        if config_cheating_mode.is_some() {
                            log_security_event(
                                "cheating_detected",
                                &config_node_id,
                                &format!("任务 {} 执行了作弊行为", task.id),
                                crate::logging::SecuritySeverity::Medium,
                            );
                        }

                        // 提交结果到共识服务器
                        if let Err(e) = consensus_client_clone.submit_result(&task.id, &result).await {
                            log_error_with_context(
                                &format!("提交任务 {} 结果失败", task.id),
                                &*e,
                            );
                        }
                        ...
                        let status_info = ProviderStatusInfo {
                            provider_id: config.node_id.clone(),
                        ...
                        // 从活跃任务中移除
                        active_tasks_clone.write().await.remove(&task.id);
                    });
                } else {
                    // 没有新任务，等待一段时间
                    tokio::time::sleep(Duration::from_millis(1000)).await;
